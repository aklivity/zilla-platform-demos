# Apache Kafka to AutoMQ Migration with Zilla Platform

This demo shows how to add flexibility to your Kafka architecture using Zilla Platform.

It demonstrates how teams can migrate from Apache Kafka to AutoMQ, without changing client code, security settings, or schemas.

## Migration Steps

* Start with **Apache Kafka** & **AutoMQ** cluster
...
* **Redeploy** the same Data Product on AutoMQ
...
- An **AutoMQ** Cluster (backed by MinIO S3 storage)
- A **Schema Registry**
- **MirrorMaker** Services (**Apache Kafka** → **AutoMQ**)
...
Using Zilla Platform explore the topics on different Kafka services (AutoMQ) to verify the messages are replicated.
...
## Step 10: Redeploy API Product on AutoMQ

`API Catalog → [catalog] → [API product] → Deploy`

Select `Target service` as `AutoMQ`.
...
```
docker compose --profile automq down
```
...
```text
docker compose --profile automq down --volumes --remove-orphans
```
